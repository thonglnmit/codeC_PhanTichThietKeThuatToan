...
void docfile(dl a[],int *n,int *w){
	FILE *f=fopen("balo3thaman.txt","r");
	fscanf (f,"%d", w);
	int i=0;
	while (!feof(f)){
		fscanf (f,"%f %f %[^\n]", &a[i].tl,&a[i].dg,a[i].ten);
		a[i].pa=0;
...
void in(dl a[],int n,int w){
	...
	printf ("|----------------------------------------------------------------|\n");
	printf ("|       Ten        | trong luong | don gia | gia tri | phuong an |\n");
	for(int i=0;i<n;i++){
		printf ("|%18s|%9.2f    |%7.2f  |%7.2f  |%5d      |\n", a[i].ten,a[i].tl,a[i].dg,a[i].gt,a[i].pa);
	}
	printf ("|----------------------------------------------------------------|\n");
	printf ("phuong an la : \n");
	float tgt,ttl;
